package com.sonin.ssmframework.springmvc.adapter;
...
public class HandlerAdapter {
    ...
    public ModelAndView handler(HttpServletRequest request, HttpServletResponse response) throws IllegalAccessException, InstantiationException {
        //1. 从Request中获取所有参数
        ...
        for (Map.Entry<String, Integer> mapP : this.methodParasMap.entrySet()) {
            ...
            if (requestParameterMap.containsKey(paraName)) {
                int index = mapP.getValue();
                String requestValues = Arrays.toString(requestParameterMap.get(paraName)).replace("\\[|\\]", "");
                // 对结果进行格式化
